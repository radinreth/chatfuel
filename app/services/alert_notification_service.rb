...
class AlertNotificationService
  ...
  def send_daily_notification
    SiteSetting.daily.enable_notification.each do |setting|
      site = setting.site
      feedbacks = StepValue.joins(variable_value: :variable).where(variables: { mark_as: 'report' }).where(site_id: site.id).where("DATE(step_values.created_at) = ?", Date.today - 1)
  ...
  def send_weekly_notification
    SiteSetting.weekly.enable_notification.each do |setting|
      site = setting.site
      feedbacks = StepValue.joins(variable_value: :variable).where(variables: { mark_as: 'report' }).where(site_id: site.id).where("DATE(step_values.created_at) >= ?", 1.week.ago)
